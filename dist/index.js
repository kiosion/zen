...
io.on('connection', (socket) => {
    ...
    socket.on('usrname-set', (room, usrname) => {
        if (!room || !usrname || (rooms[room].users[socket.id] != `tmp-${socket.id}`) || usrname == '')
            return;
        if (!rooms[room]) {
    ...
    socket.on('send-chat-message', (timestamp, room, message) => {
        ...
        if ((rooms[room]).users[socket.id] == `tmp-${socket.id}`)
            return;
